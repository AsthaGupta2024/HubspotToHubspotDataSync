...
app.post('/webhook', async (req, res) => {
    ...
    for (const data of data1) {
        ...
        try {
            ...
            // console.log("data email------------------------->", data.properties.email);
...
async function fetchNotesFromHubSpot(contactId, hubspotAccessToken) {
    ...
    try {
        ...
        console.error(`Error fetching notes for HubSpot contact ${contactId}:`, error.response ? error.response.data : error.message);
        return [];
    }
}
...
async function syncNotesWithHubSpot(email, notes) {
    ...
    const hubSpotContactId = await getHubSpotContactIdByEmail(email, hubspotAccessToken);
    ...
    for (const note of notes) {
        try {
            ...
            console.log(`Note for Contact ${hubSpotContactId} synced successfully:`, response.data);
        } catch (error) {
            console.error(`Error syncing note for Contact ${hubSpotContactId}:`, error.response ? error.response.data : error.message);
        }
    }
}
async function getHubSpotContactIdByEmail(email, accessToken) {
    ...
    try {
        ...
        console.error("Error retrieving HubSpot contact ID:", error.response ? error.response.data : error.message);
        return null;
    }
}
async function processNotesForContacts(data1, access_token) {
    for (const data of data1) {
        ...
        console.log("email-------->", email);
        if (!email) {
            console.error("Email is missing for contact:", contact.id);
            continue;
        }
...
async function fetchTasksFromHubSpot(dataId, srcaccessToken) {
    ...
    try {
        ...
        const tasks = response.data.results.filter(task => task.engagement.type === 'TASK');

        console.log(`Fetched ${tasks.length} tasks for contact ${dataId}`);
        return tasks;
    } catch (error) {
        console.error(`Error fetching tasks for HubSpot contact ${dataId}:`, error.response ? error.response.data : error.message);
        return [];
    }
}
...
async function syncTasksWithHubSpot(email, tasks) {
    ...
    for (const task of tasks) {
        try {
            ...
            console.log(`Task for Contact ${hubSpotContactId} synced successfully:`, response.data);
        } catch (error) {
            console.error(`Error syncing task for Contact ${hubSpotContactId}:`, error.response ? error.response.data : error.message);
        }
    }
}
...
async function fetchMeetingsFromHubSpot(dataId, srcAccessToken) {
    ...
    try {
        ...
        const meetings = response.data.results.filter(meeting => meeting.engagement.type === 'MEETING');

        console.log(`Fetched ${meetings.length} meetings for contact ${dataId}`);
        return meetings;
    } catch (error) {
        console.error(`Error fetching meetings for HubSpot contact ${dataId}:`, error.response ? error.response.data : error.message);
        return [];
    }
}
...
async function fetchCallsFromHubSpot(dataId, srcAccessToken) {
    ...
    try {
        ...
        const calls = response.data.results.filter(call => call.engagement.type === 'CALL');

        console.log(`Fetched ${calls.length} calls for contact ${dataId}`);
        return calls;
    } catch (error) {
        console.error(`Error fetching calls for HubSpot contact ${dataId}:`, error.response ? error.response.data : error.message);
        return [];
    }
}
...
async function syncCallsWithHubSpot(email, calls) {
    ...
    for (const call of calls) {
        try {
            ...
            console.log(`Call for Contact ${hubSpotContactId} synced successfully:`, response.data);
        } catch (error) {
            console.error(`Error syncing call for Contact ${hubSpotContactId}:`, error.response ? error.response.data : error.message);
        }
    }
}
...
async function getHubSpotContactIdByEmail(email, hubspotAccessToken) {
    try {
        ...
        console.error(`Error fetching HubSpot contact by email (${email}):`, error.response ? error.response.data : error.message);
        return null;
    }
}
...
async function fetchEmailsForContacts(data1, sourceAccessToken, targetAccessToken) {
    const results = [];
    ...
    for (const data of data1) {
        try {
            ...
            const emails = response.data.results.filter(engagement => engagement.engagement.type === 'EMAIL');
            ...
            for (const email of emails) {
                ...
                if (getResponse.results.length === 0) {
                    throw new Error(`HubSpot Contact Not Found for email: ${contactEmail}`);
                }
                 ...
                 if (email.attachments && email.attachments.length > 0) {
                    for (const attachment of email.attachments) {
                        ...
                        try {
                            ...
                            const fileData = await downloadAttachment(attachment.id, sourceAccessToken);
                            console.log("fileData----------------->",fileData);
                            console.log(`Downloaded file ${attachment.id}`);

                            // // Step 2: Upload the attachment to the target HubSpot
                            // const newFileId = await uploadAttachmentToHubSpot(fileData, attachment.name, targetAccessToken);
                            // console.log(`Uploaded file with ID ${newFileId}`);

                            
                        } catch (error) {
                            console.error(`Error syncing attachment ${attachment.id}:`, error.message);
                        }
                    }
                } else {
            ...
            console.error(`Error fetching emails for contact ${data.id}:`, error.response ? error.response.data : error.message);
        }
    }
...
const downloadAttachment = async (attachmentId, accessToken) => {
    ...
    try {
        ...
        // Ensure that the file extension exists in the response and is valid
        const extension = fileDetailsResponse.data.extension;
        const allowedExtensions = ['jpg', 'jpeg', 'png', 'pdf', 'docx', 'xlsx']; // Example allowed extensions

        if (!extension || !allowedExtensions.includes(extension.toLowerCase())) {
            throw new Error(`Invalid file extension: ${extension}`);
        }

        // Step 2: Check if the file has a public URL
        const downloadUrl = fileDetailsResponse.data.url;  // This should be the file download URL
        console.log("Download URL:", downloadUrl);  // Log the URL to check
        
        // Step 3: Download the file
        const fileData = await axios.get(downloadUrl, {
            headers: { Authorization: `Bearer ${accessToken}` },
            responseType: 'arraybuffer',  // Binary data response
        });
        console.log("File download successful:", fileData.status);

        // Step 5: Define the file path where you want to save the file
        const fileName = fileDetailsResponse.data.name || 'downloaded_file'; // Default to 'downloaded_file' if no name
        const filePath = path.join(__dirname, `${fileName}.${extension}`); // Save with correct extension

        // Step 6: Write the file data to the filesystem
        fs.writeFileSync(filePath, fileData.data);  // Save the file synchronously
        console.log(`File saved at: ${filePath}`);
        //Upload file at hubspot
        const uploadedFileId = await uploadFileToHubSpot(filePath);
        if (uploadedFileId) {
            console.log("Uploaded File ID:", uploadedFileId.id);
            console.log("infooooooo", uploadedFileId);
            await createEngagementWithAttachment(email, hubSpotContactId, uploadedFileId.id, accessToken);
        } else {
            console.log("Failed to upload file or retrieve file ID.");
        }
...
async function uploadFileToHubSpot(filePath) {
    ...
    try {
        ...
        formData.append('options', JSON.stringify({ access: 'PRIVATE', folderPath }));


        const hubspotResponse = await axios.post(hubspotUrl, formData, {
            headers: {
                'Authorization': `Bearer ${accessToken}`,
                ...formData.getHeaders(),
            },
        ...
        console.error("Error uploading file to HubSpot:", error.response ? error.response.data : error.message);
    }
}


async function createEngagementWithAttachment(email, hubSpotContactId, uploadedFileId, accessToken) {
    try {
        console.log("uploadedFileId", uploadedFileId);
        console.log("email-----------------------", email)
        console.log("hubSpotContactId", hubSpotContactId);
        console.log("accessToken", accessToken);
        // Log the engagement data for debugging
        ...
        console.log("Engagement data being sent:", JSON.stringify(engagementData, null, 2));
        ...
        console.log(`Email synced successfully with HubSpot for contact ID ${hubSpotContactId}.`, engagementResponse.data);

    } catch (error) {
        console.error(`Error syncing email for contact ID ${hubSpotContactId}:`, error.response ? error.response.data : error.message);
    }
